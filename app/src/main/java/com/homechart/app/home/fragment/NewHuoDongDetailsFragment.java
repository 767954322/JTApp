...
public class NewHuoDongDetailsFragment
    ...
    public NewHuoDongDetailsFragment() {
    }

    public NewHuoDongDetailsFragment(FragmentManager fragmentManager) {
        this.fragmentManager = fragmentManager;
    }

    @Override
    protected int getLayoutResId() {
        return R.layout.fragment_newhuodong;
    }
    ...
    protected void initExtraBundle() {
        ...
        activity_id = (String) mBundle.getString("activity_id");
    ...
    protected void initView() {
        ...
        headerView = LayoutInflater.from(activity).inflate(R.layout.header_new_huodong_details, null);
    ...
    protected void initData(Bundle savedInstanceState) {
        homeSharedPopWinPublic = new HomeSharedPopWinPublic(activity, NewHuoDongDetailsFragment.this);
        ...
        width_activity = PublicUtils.getScreenWidth(activity) - UIUtils.getDimens(R.dimen.font_30);
        buildRecycler();
        getHuoDongData();
    }
        ...
        switch (v.getId()) {
            ...
            case R.id.nav_secondary_imageButton:
                ...
                if (null != activityInfoBean) {
                    homeSharedPopWinPublic.showAtLocation(rootView.findViewById(R.id.main),
    ...
    private void buildRecycler() {
        ...
        mAdapter = new MultiItemCommonAdapter<ItemHuoDataBean>(activity, mListData, support) {
            @Override
    ...
    private void getHuoDongData() {
        OkStringRequest.OKResponseCallback callBack = new OkStringRequest.OKResponseCallback() {
            ...
            public void onErrorResponse(VolleyError volleyError) {
                ToastUtils.showCenter(activity, getString(R.string.info_get_error));
            }
    ...
    private void changeTopUI(HDDetailsBean hdDetailsBean) {
        ...
        if (ifFirst) {
            if (mTextViewPaint - (PublicUtils.getScreenWidth(activity) - UIUtils.getDimens(R.dimen.font_30)) * 4 > 0) {
                //超过四行
        ...
        layoutParams1.width = PublicUtils.getScreenWidth(activity);
        layoutParams1.height = (int) (PublicUtils.getScreenWidth(activity) / activityInfoBean.getStep_image().getRatio());
        ...
        myHuoDongJiangAdapter = new MyHuoDongJiangAdapter(listPrize, activity);
    ...
    public void addHuoDong() {
        OkStringRequest.OKResponseCallback callBack = new OkStringRequest.OKResponseCallback() {
            @Override
            public void onErrorResponse(VolleyError volleyError) {
                ToastUtils.showCenter(activity, "参加活动失败，请重新尝试");
            }
            ...
            public void onResponse(String s) {
                try {
                    ...
                    String data_msg = jsonObject.getString(ClassConstant.Parame.DATA);
                    if (error_code == 0) {
                        ToastUtils.showCenter(activity, "报名成功！返回首页收藏图片吧～");
                        getHuoDongData();
                    } else {
                        ToastUtils.showCenter(activity, error_msg);
                    }
    ...
    private void sharedItemOpen(SHARE_MEDIA share_media) {
        if(null != hdDetailsBean ){
            ...
            MyApplication.getInstance().getDefaultTracker().send(new HitBuilders.EventBuilder()
                    ...
                    .build());
            UMImage image = new UMImage(activity, hdDetailsBean.getData().getShare_info().getQq().getImage_url());
            ...
            if (share_media == SHARE_MEDIA.WEIXIN) {
                ...
                web.setTitle(hdDetailsBean.getData().getShare_info().getWechat_circle().getTitle());//标题
            } else if (share_media == SHARE_MEDIA.SINA) {
                web.setTitle(hdDetailsBean.getData().getShare_info().getWeibo().getTitle());//标题
            } else {
                web.setTitle(hdDetailsBean.getData().getShare_info().getQq().getTitle());//标题
            }
            web.setThumb(image);  //缩略图
            if (share_media == SHARE_MEDIA.WEIXIN) {
                web.setDescription(hdDetailsBean.getData().getShare_info().getWechat_friend().getDescription());//描述
            } else if (share_media == SHARE_MEDIA.WEIXIN_CIRCLE) {
                web.setDescription(hdDetailsBean.getData().getShare_info().getWechat_circle().getDescription());//描述
            } else if (share_media == SHARE_MEDIA.SINA) {
